...
export async function getAllThreads(limit = 20) {
    ...
    snapshot = snapshot.sort((a, b) => { return parseInt(b.createdOn) - parseInt(a.createdOn) }
    );

    return snapshot.slice(0, 10);
}
...
export async function updateAddressToThreadIds(address, tId, member = false, moderator = false) {
    ...
    let resp = await coll.updateOne({ '_id': address }, {
        $set: {
            "_id": address,
            "member": newMember,
...
export async function getStats() {
    ...
    let cachedTrustScoreCnt = await db.collection('cachedTrustScores').estimatedDocumentCount();
    let commentsCount = await db.collection('comments').estimatedDocumentCount();
    let distinctAuthors = await db.collection('comments').distinct("author");
    let distinctThreads = await db.collection('threads').distinct("_id");

    let authors = distinctAuthors.length;
    let threadsCount = distinctThreads.length;
